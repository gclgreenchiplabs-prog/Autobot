...
class FyersMarketDataAdapter(MarketDataAdapter):
    ...
    def _initial_state(self) -> str:
        if not self.settings.fyers_market_data_enable:
            return AdapterState.NOT_CONFIGURED.value
        if not FYERS_SDK_AVAILABLE:
            return AdapterState.SDK_NOT_INSTALLED.value
        if not (self.settings.api_key and self.settings.access_token):
            return AdapterState.NOT_CONFIGURED.value
        return AdapterState.DISCONNECTED.value

    def connect(self) -> Dict[str, Any]:
        if self._state in {AdapterState.NOT_CONFIGURED.value, AdapterState.SDK_NOT_INSTALLED.value}:
            return self.health_snapshot()
        self._state = AdapterState.FAILED.value
        return self.health_snapshot()

    def disconnect(self) -> Dict[str, Any]:
        if self._state not in {AdapterState.NOT_CONFIGURED.value, AdapterState.SDK_NOT_INSTALLED.value}:
    ...
    def readiness(self) -> Dict[str, Any]:
        reason = "official FYERS SDK unavailable" if self._state == AdapterState.SDK_NOT_INSTALLED.value else "credentials missing"
        if self._state == AdapterState.DISCONNECTED.value:
            reason = "adapter initialized but not connected"
        if self._state == AdapterState.FAILED.value:
            reason = "live connection not attempted in this sprint"
